...
if 제출:
    ...
    for 조합 in 공생합_조합:
        조건_위배 = False
        for 모험가, lv in 사용자_Lv.items():
            if lv == 0 and 조합['모험가들'].get(모험가, 0) > 0:
                조건_위배 = True
                break
        if 조건_위배:
            continue
    ...
    if 최적:
        ...
        if 다음:
            ...
            st.markdown(f"✅ 필요한 모래양: **{다음['모래소모량']:,}개**")
            headers = "".join([f"<th>{모험가}</th>" for 모험가 in 다음["모험가들"]])
            rows = "".join([f"<td>Lv {lv}</td>" for lv in 다음["모험가들"].values()])
            st.markdown(f"""
            <table style='width:100%; text-align:left;'>
                <tr>{headers}</tr>
                <tr>{rows}</tr>
